...
⚙️ How It Works
Initialization: The bot starts, loads configuration from config.json, and sets up logging.
Scanning: It periodically (or on manual /update) scans your configured media folders.
Comparison: It compares the current state of your folders with the last saved state (folder_list.json).
Metadata & Posters: For any new folders, it parses the folder name to extract title and year, queries the OMDb API for metadata, and downloads the corresponding poster. This data is cached in media_metadata.json.
Notifications: Based on detected changes (additions or removals), it constructs rich Telegram messages (with posters for new content) and sends them to your designated chat.
Persistence: The updated folder state and media metadata cache are saved back to their respective JSON files for the next run.
Heartbeat: If enabled, a periodic message is sent to confirm the bot's operational status.
Service Mode: NSSM ensures the Python script runs continuously in the background, handles restarts, and redirects output to dedicated logs.
...
1. Project Setup
Create Project Directory: Create a dedicated folder for your bot, e.g., C:\Scripts\Plex_Updater.
Download Files: Place all the bot's Python script files (main.py, config_manager.py, telegram_notifier.py, media_scraper.py, telegram_bot_handler.py) into this directory.
Install Python Dependencies: Open a command prompt or PowerShell, navigate to your project directory, and run:
...
This step allows the bot to run continuously in the background.
    ...
    Navigate to NSSM: In the command prompt, cd into the win64 (or win32) folder where you extracted NSSM.
cd "C:\NSSM\nssm-2.24\win64"

(Adjust path to your NSSM location)

    Install the Service: Run the NSSM installer GUI:
...
.\nssm install PlexUpdaterBot
    ...
    Click "Install Service". You should see a success message.

5. Start the Service
    Open Windows Services Manager: Press Win + R, type services.msc, and press Enter.
    Find your service: Locate Plex Updater Bot in the list.
    Start the service: Right-click on Plex Updater Bot and select Start. The status should change to "Running."
